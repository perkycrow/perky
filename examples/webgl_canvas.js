...
function setupUI(container) {
    ...
    addButtonFolder(controlPane, 'Shapes', [
        ...
        },
        {
            ...
            action: () => stressTest(100)
        }, {
            title: 'Stress Test (500 shapes)',
            action: () => stressTest(500)
        }
    ])
    ...
    await new Promise((resolve, reject) => {
        ...
        logoImage.src = 'data:image/svg+xml,' + encodeURIComponent(`
            <svg width="100" height="100" xmlns="http://www.w3.org/2000/svg">
                ...
                <text x="50" y="60" font-family="Arial" font-size="40" fill="white" text-anchor="middle">GL</text>
            </svg>
        `)
    })
...
function stressTest(count) {
    const logger = document.querySelector('.logger')?.logger
    const stressGroup = new Group2D()

    for (let i = 0; i < count; i++) {
        const shape = createRandomShape()
        stressGroup.addChild(shape)
    }

    scene.addChild(stressGroup)

    renderer.render(scene)
    updateStats()

    logger?.info(`Stress test: added ${count} shapes. Check FPS!`)

    setTimeout(() => {
        scene.remove(stressGroup)
        renderer.render(scene)
        updateStats()
        logger?.info('Stress test shapes removed')
    }, 5000)
}
